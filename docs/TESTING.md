...
| `nebulad` unit tests (13) | JPEG/H264 encoders produce valid decodable bitstreams (SOI/Annex-B, keyframes, bitrate re-init), AIMD controller (backlog cut, fps floor, bufferbloat, recovery), PIN single-use/rotation/lockout, trust store enroll/verify/revoke/persistence/transcript-MITM, test-pattern motion, size parsing |
| Rust e2e (4, real sockets) | full pairing → H.264 streaming (seq order, changing payloads, keyframe first), encrypted ping/pong, wrong PIN rejected + PIN rotation, token reconnect, input-grant live flow, revocation kick + token invalidation, client-side fingerprint pinning |
| Node compat test | the **actual web-viewer session code** (esbuild-bundled) against the real host under Node 22 WebCrypto: pair, decrypt frames, JPEG magic, encrypted ping/pong, token reconnect, wrong PIN |
| Browser E2E (Chromium) | UI pairing, WebCodecs H.264 1280×720 with changing canvas pixels, stats overlay showing *measured* e2e latency, input grant flow reaching the host input sink, panel PIN/QR/client list, profile switch |
