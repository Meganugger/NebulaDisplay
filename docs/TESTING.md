...
|---|---|
| `ndsp-protocol` unit tests (24) | message serde + stable tags + forward-compat, ECDH key agreement, wrong-PIN key mismatch, AEAD tamper rejection, envelope replay/direction/AAD attacks, media framing, beacon parsing, **NDSP-PAKE v1** (agreement, wrong-PIN divergence, channel-binding divergence, invalid-share rejection, share freshness, RFC 9380 hash-to-curve vector) |
| `nebulad` unit tests (30) | JPEG/H264 encoders produce valid decodable bitstreams (SOI/Annex-B, keyframes), **runtime bitrate/fps changes without stream reset**, resolution change rebuild+IDR, BGRA→I420 byte-exact vs reference, adaptive controller (backlog cut, decrease cooldown/hysteresis, sticky fps + floor-only fps drop, sustained-RTT bufferbloat, recovery, fps restore), PIN single-use/rotation/lockout, trust store enroll/verify/revoke/persistence/transcript-MITM, keystore protect/unprotect roundtrip (+ foreign-DPAPI-blob error), clipboard backend sequence tracking, TLS identity creation/persistence, test-pattern motion, size parsing |
| Rust e2e (10, real sockets) | full **PAKE** pairing → H.264 streaming (seq order, changing payloads, keyframe first), encrypted ping/pong, wrong PIN rejected + PIN rotation, token reconnect, input-grant live flow, revocation kick + token invalidation, client-side fingerprint pinning, **legacy pairing compat** (works by default, refused with `allow_legacy_pairing=false` while PAKE still passes), **clipboard sync** (deny-by-default, live grant, both directions, echo suppression, size-cap refusal), **pinned-fingerprint TLS** (right pin streams, wrong pin fails in the TLS handshake), **video keeps full rate under a 240 Hz input flood** (pipeline-independence regression guard) |
...
| Windows CI job | compiles + clippy-gates all `cfg(windows)` code (DXGI incl. cursor compositing + cursor-only readback skip, IddCx multi-ring consumer, QueryDisplayConfig input mapping, SendInput multi-monitor mapping incl. layout-aware `VkKeyScanW` keys, Win32 clipboard backend, DPAPI keystore, MF hardware encoder incl. ROI hints, tray) |
...
- [ ] layout-mismatch typing: QWERTY viewer → AZERTY host (and reverse) types
      the characters pressed, Ctrl+C/V follow the character not the position
- [ ] clipboard denied by default; grant → both directions sync on a real
      Windows host (Win32 clipboard); revoke stops it live
- [ ] `--tls` host: browser connects after accepting the cert; desktop viewer
      connects with `--tls-pin` and refuses a wrong pin
